...
async function createCardForOwner(ownerType: OwnerType, owner: any, issuedBy: any) {
  ...
  return IDCard.create({
    ownerId: owner.userId?._id || owner.userId,
    ownerType,
    cardNumber,
    cardType: ownerType,
    photoUrl: owner.userId?.avatar || '',
...
export const generateStudentIdCard = async (req: Request, res: Response) => {
  try {
    const student = await Student.findOne({ _id: req.params.studentId, institutionId: (req as any).user?.institutionId }).populate('userId').populate('classId').populate('sectionId').populate('institutionId');
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const card = await createCardForOwner('student', student, (req as any).user?._id || (req as any).user?.id);
    const pdf = await createBasicPdf({ title: 'Student ID Card', institutionName: (student.institutionId as any)?.name, name: (student.userId as any)?.name, ownerType: 'student', cardNumber: card.cardNumber, className: (student.classId as any)?.name, sectionName: (student.sectionId as any)?.name, rollNumber: (student as any).rollNumber, qrCodeData: card.qrCodeData });
...
export const generateTeacherIdCard = async (req: Request, res: Response) => {
  try {
    const teacher = await Teacher.findOne({ _id: req.params.teacherId, institutionId: (req as any).user?.institutionId }).populate('userId').populate('institutionId');
    if (!teacher) return res.status(404).json({ message: 'Teacher not found' });
    const card = await createCardForOwner('teacher', teacher, (req as any).user?._id || (req as any).user?.id);
    const pdf = await createBasicPdf({ title: 'Teacher ID Card', institutionName: (teacher.institutionId as any)?.name, name: (teacher.userId as any)?.name, ownerType: 'teacher', cardNumber: card.cardNumber, designation: teacher.designation, qrCodeData: card.qrCodeData });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${card.cardNumber}.pdf"`);
    return res.send(pdf);
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
...
export const generateStaffIdCard = async (req: Request, res: Response) => {
  try {
    const staff = await Staff.findOne({ _id: req.params.staffId, institutionId: (req as any).user?.institutionId }).populate('userId').populate('institutionId');
    if (!staff) return res.status(404).json({ message: 'Staff not found' });
    const card = await createCardForOwner('staff', staff, (req as any).user?._id || (req as any).user?.id);
    const pdf = await createBasicPdf({ title: 'Staff ID Card', institutionName: (staff.institutionId as any)?.name, name: (staff.userId as any)?.name, ownerType: 'staff', cardNumber: card.cardNumber, designation: staff.designation, qrCodeData: card.qrCodeData });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${card.cardNumber}.pdf"`);
    return res.send(pdf);
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
...
export const bulkGenerateIdCards = async (req: Request, res: Response) => {
  try {
    const { type = 'student', ids = [] } = req.body;
    if (!Array.isArray(ids) || ids.length === 0) return res.status(400).json({ message: 'ids required' });
    const ownerType = type === 'teacher' || type === 'staff' || type === 'head' ? type : 'student';
    const results: any[] = [];
    for (const ownerId of ids.slice(0, 50)) {
      const owner = await getOwner(ownerType, ownerId, (req as any).user?.institutionId);
      if (!owner) { results.push({ ownerId, error: 'not found' }); continue; }
      const card = await createCardForOwner(ownerType, owner, (req as any).user?._id || (req as any).user?.id);
      results.push({ ownerId, cardNumber: card.cardNumber });
    }
    return res.json({ message: `Generated ${results.length} cards`, results });
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
};

export const downloadIdCard = async (req: Request, res: Response) => {
  try {
    const card = await IDCard.findOne({ _id: req.params.id, institutionId: (req as any).user?.institutionId }).populate('ownerId').populate('institutionId');
    if (!card) return res.status(404).json({ message: 'Card not found' });
    if (!canReadCard(req, card)) return res.status(403).json({ message: 'Access denied. Cannot download this card.' });
    card.downloadCount = (card.downloadCount || 0) + 1;
    card.lastDownloadedAt = new Date();
    await card.save();
    const pdf = await createBasicPdf({ title: 'ID Card', institutionName: (card.institutionId as any)?.name, name: (card.ownerId as any)?.name, ownerType: card.ownerType, cardNumber: card.cardNumber, qrCodeData: card.qrCodeData, validityStart: card.validityStart?.toISOString().slice(0, 10), validityEnd: card.validityEnd?.toISOString().slice(0, 10) });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${sanitizeFilename(card.cardNumber)}.pdf"`);
    return res.send(pdf);
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
};
...
export const emailIdCard = async (req: Request, res: Response) => {
  try {
    ...
    const card = await IDCard.findOne({ _id: req.params.id, institutionId: (req as any).user?.institutionId }).populate('ownerId').populate('institutionId');
    if (!card) return res.status(404).json({ message: 'Card not found' });
    const pdf = await createBasicPdf({ title: 'ID Card', institutionName: (card.institutionId as any)?.name, name: (card.ownerId as any)?.name, ownerType: card.ownerType, cardNumber: card.cardNumber, qrCodeData: card.qrCodeData });
    const sent = await sendEmail({ to, subject: `ID Card ${card.cardNumber}`, text: 'Attached ID card', html: '<p>Attached ID card.</p>', attachments: [{ filename: `${card.cardNumber}.pdf`, content: pdf, contentType: 'application/pdf' }] });
...
export const verifyByQRCode = async (req: Request, res: Response) => {
  try {
    if (!req.body.code) return res.status(400).json({ message: 'code required' });
    const card = await IDCard.findOne({ qrCodeData: req.body.code }).populate('ownerId');
    if (!card) return res.status(404).json({ message: 'Card not found' });
...
export const renewIdCard = async (req: Request, res: Response) => {
  try {
    const card = await IDCard.findOne({ _id: req.params.id, institutionId: (req as any).user?.institutionId });
    if (!card) return res.status(404).json({ message: 'Card not found' });
    const extendYears = Number(req.body.extendYears || 1);
    if (req.body.action === 'request') card.status = 'pending-renewal';
    else if (req.body.action === 'reject') card.status = 'active';
    else { card.validityEnd = moment(card.validityEnd).add(extendYears, 'years').toDate(); card.status = 'active'; }
    await card.save();
    return res.json({ message: 'Renewal updated', card });
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
};

export const idCardStats = async (req: Request, res: Response) => {
  try {
    const match: any = { institutionId: (req as any).user?.institutionId };
    const [total, byStatus, byType] = await Promise.all([
      IDCard.countDocuments(match),
    ...
    return res.json({ total, totalIssued: total, studentCards: typeCount('student'), teacherCards: typeCount('teacher'), staffCards: typeCount('staff'), expiredCards: statusCount('expired'), pendingRenewals: statusCount('pending-renewal'), downloads: byType.reduce((sum: number, item: any) => sum + (item.downloads || 0), 0), byStatus, byType });
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
};

export const getAllIdCards = async (req: Request, res: Response) => {
  try {
    const cards = await IDCard.find({ institutionId: (req as any).user?.institutionId }).populate('ownerId').sort({ createdAt: -1 }).limit(300);
...
export const getIdCardById = async (req: Request, res: Response) => {
  try {
    const card = await IDCard.findOne({ _id: req.params.id, institutionId: (req as any).user?.institutionId }).populate('ownerId').populate('institutionId');
    if (!card) return res.status(404).json({ message: 'Card not found' });
    if (!canReadCard(req, card)) return res.status(403).json({ message: 'Access denied. Cannot view this card.' });
...
export const searchIdCardOwners = async (req: Request, res: Response) => {
  try {
    ...
    let people: any[] = [];
    if (type === 'teacher') people = await Teacher.find({ institutionId: (req as any).user?.institutionId }).populate('userId', 'name avatar email phone').limit(100).lean();
    else if (type === 'staff') people = await Staff.find({ institutionId: (req as any).user?.institutionId }).populate('userId', 'name avatar email phone').limit(100).lean();
    else people = await Student.find({ institutionId: (req as any).user?.institutionId }).populate('userId', 'name avatar email phone').populate('classId', 'name grade').populate('sectionId', 'name').limit(200).lean();
...
export const generateIdCardRecord = async (req: Request, res: Response) => {
  try {
    const ownerType: OwnerType = req.body.ownerType === 'teacher' || req.body.ownerType === 'staff' || req.body.ownerType === 'head' ? req.body.ownerType : 'student';
    const owner = await getOwner(ownerType, req.body.ownerId, (req as any).user?.institutionId);
    if (!owner) return res.status(404).json({ message: 'Owner not found' });
    const card = await createCardForOwner(ownerType, owner, (req as any).user?._id || (req as any).user?.id);
    return res.status(201).json({ message: 'ID card generated', card });
  } catch (error) { return res.status(500).json({ message: 'Server error', error }); }
};

export const renderCardPdf = async (req: Request, res: Response) => {
  try {
    const pdf = await createBasicPdf(req.body || {});
    res.setHeader('Content-Type', 'application/pdf');
