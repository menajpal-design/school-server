...
export const listBooks = async (query: any = {}, institutionId?: Types.ObjectId) => Promise.all((await Book.find(buildBookQuery(query, institutionId)).sort({ title: 1 })).map((book) => attachBookQrCode(book)));
