...
export async function resolveActorScope(user: any, _tenantConnection?: any) {
  ...
  const scope: any = {
    ...
    teacherId: undefined,
    staffId: undefined,
    assignedClassIds: idList(user?.assignedClasses || user?.classIds || []),
    assignedSectionIds: idList(user?.assignedSections || user?.sectionIds || []),
    assignedSubjectIds: idList(user?.subjects || user?.assignedSubjects || user?.subjectIds || []),
    isSchoolLeader: isSchoolLeader(user),
  ...
  if (user.role === 'student') {
    ...
    if (student) {
      ...
      scope.assignedClassIds = idList([...scope.assignedClassIds, student.classId]);
      scope.assignedSectionIds = idList([...scope.assignedSectionIds, student.sectionId]);
    }
  }
  ...
  if (teacherRoles.includes(user.role)) {
    ...
    if (teacher) {
      ...
      scope.assignedClassIds = idList([...scope.assignedClassIds, ...(teacher.assignedClasses || [])]);
      scope.assignedSubjectIds = idList([...scope.assignedSubjectIds, ...(teacher.subjects || [])]);
    }
  }
...
export function canPerform(action: Action, user: any) {
  ...
  switch (action) {
    ...
    case 'result:publish':
    case 'exam:publish':
    case 'routine:publish': return user.role === 'head';
    case 'exam:create':
    case 'exam:update':
    case 'exam:delete':
    case 'class:create':
...
export const requireAction = (action: Action) => (req: any, res: Response, next: NextFunction) => {
  if (!req.user) return res.status(401).json({ message: 'Authentication required.' });
  if (!canPerform(action, req.user)) return res.status(403).json({ message: 'Access denied.' });
  return next();
};
...
export function scopedClassQuery(scope: any, base: any = {}) {
  if (scope.isSchoolLeader || scope.isSystemAdmin) return base;
  if (scope.assignedClassIds.length) return { ...base, _id: { $in: scope.assignedClassIds } };
  return null;
}

export function scopedSubjectQuery(scope: any, base: any = {}) {
  if (scope.isSchoolLeader || scope.isSystemAdmin) return base;
  if (scope.role === 'student' || scope.role === 'parent' || scope.role === 'class_teacher') {
    if (!scope.assignedClassIds.length) return null;
    return { ...base, classId: { $in: scope.assignedClassIds } };
  }
  if (teacherRoles.includes(scope.role)) {
    if (scope.assignedSubjectIds.length) return { ...base, _id: { $in: scope.assignedSubjectIds } };
    if (scope.assignedClassIds.length) return { ...base, classId: { $in: scope.assignedClassIds } };
  }
  return null;
}

export function scopedExamQuery(scope: any, base: any = {}) {
  if (scope.isSchoolLeader || scope.isSystemAdmin) return base;
  const query = { ...base };
  if (['student', 'parent'].includes(scope.role)) {
    if (!scope.assignedClassIds.length) return null;
