...
export const isSystemAdmin = (user: any) => systemAdminRoles.includes(normalizeRole(user?.role));
export const isSchoolLeader = (user: any) => schoolLeaderRoles.includes(normalizeRole(user?.role)) || isSystemAdmin(user);
export const isHeadPublisher = (user: any) => headPublishRoles.includes(normalizeRole(user?.role));
...
export function canPerform(action: Action, user: any) {
  ...
  switch (action) {
    case 'result:create':
    case 'result:update': return resultEntryRoles.includes(role);
    case 'result:delete': return role === 'head';
    case 'result:approve_assistant': return ['assistant_head', 'head'].includes(role);
    case 'result:approve_head':
    case 'result:publish': return ['head', 'assistant_head'].includes(role);
    case 'exam:create':
    case 'exam:update':
    case 'exam:delete': return ['head', 'assistant_head'].includes(role);
    case 'exam:approve': return ['assistant_head', 'head'].includes(role);
    case 'exam:publish':
    case 'routine:publish': return ['head', 'assistant_head'].includes(role);
    case 'class:create':
    case 'class:update':
    case 'class:delete':
    case 'subject:create':
    ...
    case 'sms:monitor': return ['head', 'assistant_head'].includes(role);
    case 'attendance:mark': return ['head', 'assistant_head', 'class_teacher'].includes(role);
    case 'leave:create': return ['student', 'parent', 'teacher', 'class_teacher', 'subject_teacher', 'staff', 'finance_officer', 'librarian'].includes(role);
    case 'leave:approve': return ['head', 'assistant_head', 'class_teacher'].includes(role);
    case 'library:manage': return ['head', 'assistant_head', 'librarian', 'staff'].includes(role);
    case 'idcard:generate':
    case 'idcard:manage': return ['head', 'assistant_head'].includes(role);
    case 'idcard:scan': return ['head', 'assistant_head', 'class_teacher'].includes(role);
...
export const examPublishGuard = (req: any, res: Response, next: NextFunction) => {
  if (!req.user) return res.status(401).json({ message: 'Authentication required.' });
  if (schoolLeaderAdminRoles.includes(normalizeRole(req.user.role)) || hasPermission(req.user, 'exam:publish') || hasPermission(req.user, 'routine:publish')) return next();
...
export const examApproveGuard = (req: any, res: Response, next: NextFunction) => {
  if (!req.user) return res.status(401).json({ message: 'Authentication required.' });
  if (schoolLeaderAdminRoles.includes(normalizeRole(req.user.role)) || hasPermission(req.user, 'exam:approve')) return next();
...
export async function teacherAssignedExamScope(user: any, base: any = {}) {
  ...
  const role = normalizeRole(user?.role);
  if (!['teacher', 'subject_teacher'].includes(role)) return null;
...
export async function classTeacherExamScope(user: any, base: any = {}) { const scope = await resolveActorScope(user); if (normalizeRole(user?.role) !== 'class_teacher') return null; if (!scope.assignedClassIds.length) return null; return { ...base, classId: { $in: scope.assignedClassIds } }; }
export async function publishedOwnClassExamScope(user: any, base: any = {}) { const scope = await resolveActorScope(user); if (normalizeRole(user?.role) !== 'student' || !scope.assignedClassIds.length) return null; return { ...base, classId: { $in: scope.assignedClassIds }, isPublished: true }; }
export async function childClassExamScope(user: any, base: any = {}) { const scope = await resolveActorScope(user); if (normalizeRole(user?.role) !== 'parent' || !scope.assignedClassIds.length) return null; return { ...base, classId: { $in: scope.assignedClassIds }, isPublished: true }; }
export async function examReadScope(user: any, base: any = {}) { if (!user) return null; const role = normalizeRole(user.role); if (blockedAcademicDataRoles.includes(role)) return null; if (isSchoolLeader(user) || isSystemAdmin(user)) return base; if (['teacher', 'subject_teacher'].includes(role)) return teacherAssignedExamScope(user, base); if (role === 'class_teacher') return classTeacherExamScope(user, base); if (role === 'student') return publishedOwnClassExamScope(user, base); if (role === 'parent') return childClassExamScope(user, base); return null; }

export async function assignedSubjectResultScope(user: any, classId?: any, subjectId?: any) { if (isHeadPublisher(user) || isSchoolLeader(user)) return { allowed: true, scope: await resolveActorScope(user) }; const scope = await resolveActorScope(user); if (!teacherRoles.includes(normalizeRole(user.role))) return { allowed: false, scope }; const classOk = classId ? scope.assignedClassIds.includes(toId(classId)) : scope.assignedClassIds.length > 0; const subjectOk = subjectId ? scope.assignedSubjectIds.includes(toId(subjectId)) : scope.assignedSubjectIds.length > 0; return { allowed: Boolean(classOk && subjectOk), scope }; }
export async function ownResultScope(user: any) { const scope = await resolveActorScope(user); if (normalizeRole(user?.role) !== 'student' || !scope.studentId) return null; return { institutionId: scope.institutionId, studentId: scope.studentId }; }
export async function childResultScope(user: any, studentId?: any) { const scope = await resolveActorScope(user); if (normalizeRole(user?.role) !== 'parent' || !scope.childStudentIds.length) return null; const allowedChildIds = scope.childStudentIds.map(String); if (studentId && !allowedChildIds.includes(String(studentId))) return null; return { institutionId: scope.institutionId, studentId: studentId || { $in: scope.childStudentIds } }; }
