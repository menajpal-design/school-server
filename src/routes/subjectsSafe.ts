...
async function enrich(subjects: any[]) {
  ...
  const teacherIds = [...new Set(subjects.map((item) => String(item.teacherId?._id || item.teacherId || '')).filter(Boolean))];
  ...
  return subjects.map((subject: any) => {
    const raw = typeof subject.toObject === 'function' ? subject.toObject() : subject;
    return {
      ...raw,
...
router.delete('/:id', authenticate, canManageAcademic(), async (req: any, res) => {
  try {
    const subject = await Subject.findOneAndDelete({ _id: req.params.id, institutionId: req.user.institutionId });
    if (!subject) return res.status(404).json({ message: 'Subject not found' });
    await ClassModel.updateMany({ institutionId: req.user.institutionId }, { $pull: { subjects: subject._id } }).catch(() => undefined);
    await Teacher.updateMany({ institutionId: req.user.institutionId }, { $pull: { subjects: subject._id } }).catch(() => undefined);
