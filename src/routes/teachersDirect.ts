...
router.post('/', async (req: any, res) => {
  try {
    ...
    const primaryClassTeacherClassId = classTeacherClassId || (role === 'class_teacher' ? classIds[0] : undefined);
    ...
    if (role === 'class_teacher') await assignClassTeacher(M, req.user.institutionId, user._id, primaryClassTeacherClassId);
...
router.put('/:id', async (req: any, res) => {
  try {
    ...
    if (rawId.startsWith('user-')) { const userId = rawId.replace(/^user-/, ''); await syncTeacherProfiles(req, M); teacher = await M.Teacher.findOne({ userId, institutionId: req.user.institutionId }); }
    else teacher = await M.Teacher.findOne({ _id: rawId, institutionId: req.user.institutionId });
    ...
    await teacher.save();
    if (role === 'class_teacher') await assignClassTeacher(M, req.user.institutionId, teacher.userId, classTeacherClassId || classIds[0]);
    else await M.Class.updateMany({ institutionId: req.user.institutionId, classTeacherId: teacher.userId }, { $unset: { classTeacherId: '' } });
...
router.delete('/:id', async (req: any, res) => {
  try {
    ...
    await M.Class.updateMany({ institutionId: req.user.institutionId, classTeacherId: teacher.userId }, { $unset: { classTeacherId: '' } });
