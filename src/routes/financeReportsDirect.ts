...
async function models(req: any) {
  ...
  return { Fee: model('Fee', Fee), Payment: model('Payment', Payment), Salary: model('Salary', Salary), Student: model('Student', Student) };
}
...
const dateRange = (query: any) => {
  const now = new Date();
  const start = query.startDate ? new Date(`${query.startDate}T00:00:00.000Z`) : new Date(now.getFullYear(), now.getMonth(), 1);
  const endBase = query.endDate ? new Date(`${query.endDate}T00:00:00.000Z`) : now;
...
router.get('/', async (req: any, res) => {
  try {
    ...
    const [paymentsRaw, duesRaw, salariesRaw, allFeesRaw] = await Promise.all([
    ...
    const collections = await enrichStudents(paymentsRaw);
    const dues = await enrichStudents(duesRaw);
    ...
    for (const fee of allFeesRaw as any[]) {
      const type = fee.type || 'other';
      typeMap.set(type, (typeMap.get(type) || 0) + Number(fee.amount || 0));
    }
    const byType = [...typeMap.entries()].map(([name, value]) => ({ name, value }));

    const summary = {
      totalCollection: paymentsRaw.reduce((sum: number, item: any) => sum + Number(item.amount || 0), 0),
      totalDue: duesRaw.reduce((sum: number, item: any) => sum + Number(item.amount || 0), 0),
      totalSalary: salariesRaw.reduce((sum: number, item: any) => sum + Number(item.netSalary || 0), 0),
      collectionCount: paymentsRaw.length,
      dueCount: duesRaw.length,
    ...
    res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to load finance reports', error });
  }
});
