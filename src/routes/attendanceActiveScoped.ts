...
async function models(req: any) { const uri = await activeUri(req); if (!connections.has(uri)) connections.set(uri, mongoose.createConnection(uri, { maxPoolSize: 5, serverSelectionTimeoutMS: 15000, connectTimeoutMS: 15000, socketTimeoutMS: 30000, retryWrites: true }).asPromise()); const c = await connections.get(uri)!; await c.db.admin().ping(); const model = (name: string, base: any) => c.models[name] || c.model(name, base.schema, base.collection?.name || name); return { Student: model('Student', Student), Teacher: model('Teacher', Teacher), Staff: model('Staff', Staff), Attendance: model('Attendance', Attendance), Class: model('Class', Class), Section: model('Section', Section) }; }
...
router.get('/student/:id', authenticate, async (req: any, res) => {
  try { const M = await models(req); const student: any = await M.Student.findOne({ _id: req.params.id, institutionId: req.user.institutionId, isActive: { $ne: false } }).populate('classId', 'name grade').populate('sectionId', 'name').lean(); if (!student) return res.status(404).json({ message: 'Student not found' }); await assertClass(M, req, student.classId?._id || student.classId); const q: any = { institutionId: req.user.institutionId, studentId: student._id, userType: 'student' }; const range = rangeOf(req.query); if (range) q.date = range; const attendance = await M.Attendance.find(q).sort({ date: 1 }).lean(); const [profile] = await enrichUsers([student]); return res.json({ attendance, profile, summary: summary(attendance), debug: { source: 'active-school-db', count: attendance.length } }); } catch (error: any) { return res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to load student attendance', error }); }
});

router.get('/person/:type/:id', authenticate, async (req: any, res) => {
  try { const M = await models(req); const type = String(req.params.type || '').toLowerCase(); const q: any = { institutionId: req.user.institutionId, userId: req.params.id, userType: type }; const range = rangeOf(req.query); if (range) q.date = range; const attendance = await M.Attendance.find(q).sort({ date: 1 }).lean(); return res.json({ attendance, summary: summary(attendance), debug: { source: 'active-school-db', count: attendance.length } }); } catch (error: any) { return res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to load person attendance', error }); }
});

router.get('/', authenticate, async (req: any, res, next) => {
  try { if (!req.query.date && !req.query.startDate && !req.query.endDate && !req.query.classId && !req.query.userType) return next(); const M = await models(req); const q: any = { institutionId: req.user.institutionId }; if (req.query.classId) { await assertClass(M, req, req.query.classId); q.classId = req.query.classId; } if (req.query.sectionId) q.sectionId = req.query.sectionId; if (req.query.userType) q.userType = req.query.userType; const range = rangeOf(req.query); if (range) q.date = range; const attendance = await M.Attendance.find(q).populate('studentId', 'rollNumber guardianName').populate('classId', 'name grade').populate('sectionId', 'name').sort({ date: -1 }).lean(); return res.json({ attendance, summary: summary(attendance), debug: { source: 'active-school-db', count: attendance.length } }); } catch (error: any) { return res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to load attendance', error }); }
});

router.post('/mark', authenticate, async (req: any, res) => {
  try { const M = await models(req); const records = Array.isArray(req.body.records) ? req.body.records : [req.body]; const { date } = dayRange(req.body.date); if (!records.length) return res.status(400).json({ message: 'No attendance records provided.' }); await assertClass(M, req, req.body.classId || records[0]?.classId); const saved: any[] = []; for (const row of records) { const userType = row.userType || (row.studentId ? 'student' : 'teacher'); if (userType === 'student') await assertClass(M, req, row.classId || req.body.classId); const filter: any = { institutionId: req.user.institutionId, date, userType }; if (userType === 'student') { filter.studentId = row.studentId; filter.classId = row.classId || req.body.classId; filter.sectionId = row.sectionId || req.body.sectionId || undefined; } else filter.userId = row.userId; const doc = await M.Attendance.findOneAndUpdate(filter, { ...filter, status: row.status || 'absent', markedBy: req.user._id, markedAt: new Date() }, { upsert: true, new: true, setDefaultsOnInsert: true }); saved.push(doc); } return res.json({ message: 'Attendance saved successfully.', attendance: saved, summary: summary(saved), debug: { source: 'active-school-db', count: saved.length } }); } catch (error: any) { return res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to save attendance', error }); }
});
