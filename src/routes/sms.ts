...
router.get('/head/monthly', authorize('head', 'assistant_head'), async (req, res) => {
  try {
    ...
    const [institution, logs, students] = await Promise.all([
      ...
      Student.find({ institutionId, isActive: true })
    ...
    const recipients = students.map((student: any) => {
      ...
      return {
        ...
        studentName: student.userId?.name || student.guardianName || 'Student',
        rollNumber: student.rollNumber,
        guardianName: student.guardianName,
        guardianPhone: student.guardianPhone,
        smsSent: Boolean(phone && sentPhoneSet.has(phone)),
        sentCount: matchingLogs.length,
        lastSentAt: matchingLogs[0]?.sentAt || null,
      };
    });
