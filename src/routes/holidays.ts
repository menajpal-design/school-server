...
const DEFAULT_WEEKLY_DAYS: number[] = [];
...
const findWeeklyDays = (obj: any): any => {
  ...
  for (const key of ['weeklyDays', 'weeklyOffDays', 'weeklyOff', 'weeklyHoliday', 'weeklyHolidays', 'weekendDays', 'weekend', 'weekends', 'offDays', 'holidayDays', 'closedDays', 'schoolClosedDays', 'schoolWeeklyOffDays', 'schoolWeekendDays']) if (obj[key] !== undefined) return obj[key];
...
const settingsWeeklyDays = async (institutionId: any) => {
  const settings = await SiteSetting.find({ institutionId, key: { $in: ['app_control_settings', 'site_config', 'holiday_settings', 'attendance_settings', 'settings'] } }).lean();
...
const weekendHolidays = (year: number, weeklyDays: number[] = []): HolidaySeed[] => {
...
const ensureBangladeshHolidays = async (institutionId: any, year: number, createdBy?: any, includeWeekends = true, weeklyDays: number[] = []) => {
...
router.get('/', authenticate, async (req: any, res) => {
  try { const year = Number(req.query.year || new Date().getFullYear()); const autoSeed = req.query.autoSeed !== 'false'; const weeklyDays = req.query.weeklyDays ? normalizeWeeklyDays(req.query.weeklyDays) : await settingsWeeklyDays(req.user.institutionId); if (autoSeed) await ensureBangladeshHolidays(req.user.institutionId, year, req.user._id, true, weeklyDays); const start = new Date(year, 0, 1); const end = new Date(year, 11, 31, 23, 59, 59, 999); const query: any = { institutionId: req.user.institutionId, startDate: { $lte: end }, endDate: { $gte: start }, isEnabled: { $ne: false }, isSchoolClosed: { $ne: false } }; if (req.query.type) query.type = req.query.type; const holidays = await Holiday.find(query).sort({ startDate: 1 }).lean(); res.json({ holidays, autoSeeded: autoSeed, weeklyDays }); } catch (error) { res.status(500).json({ message: 'Failed to load holidays', error }); }
});
