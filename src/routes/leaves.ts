...
const populateLeave = () => LeaveApplication.find().populate({ path: 'studentId', populate: { path: 'userId', select: 'name email avatar' } }).populate('userId', 'name email avatar role').populate('classId', 'name grade').populate('sectionId', 'name').populate('approvedBy', 'name role').populate('reviewedBy', 'name role');
const buildDateList = (start: Date, end: Date) => { const dates: Date[] = []; const current = new Date(start.getFullYear(), start.getMonth(), start.getDate()); const finish = new Date(end.getFullYear(), end.getMonth(), end.getDate()); while (current <= finish) { dates.push(new Date(current)); current.setDate(current.getDate() + 1); } return dates; };
const getOwnStudentIds = async (req: any) => { if (req.user.role === 'student') { const student = await Student.findOne({ institutionId: req.user.institutionId, userId: req.user._id }).select('_id').lean(); return student ? [student._id] : []; } if (req.user.role === 'parent') { const parent = await Parent.findOne({ institutionId: req.user.institutionId, userId: req.user._id }).lean(); return parent?.children || []; } return []; };
const getAssignedClassIds = async (req: any) => { if (req.user.role !== 'class_teacher') return []; const teacher = await Teacher.findOne({ institutionId: req.user.institutionId, userId: req.user._id }).select('assignedClasses').lean(); return (teacher?.assignedClasses || []).map((id: any) => String(id)); };
const canReviewLeave = async (req: any, leave: any) => { if (['head', 'assistant_head', 'admin', 'super_admin'].includes(req.user.role)) return true; if (req.user.role !== 'class_teacher') return false; const assignedClassIds = await getAssignedClassIds(req); return assignedClassIds.includes(String(leave.classId?._id || leave.classId)); };
...
router.get('/', async (req: any, res) => {
  try {
    ...
    if (req.user.role === 'class_teacher') { const assignedClassIds = await getAssignedClassIds(req); query.classId = { $in: assignedClassIds }; if (!req.query.status) query.status = 'pending'; }
    else if (!approvalRoles.includes(req.user.role)) { const ownIds = await getOwnStudentIds(req); query.studentId = { $in: ownIds }; }
    const leaves = await populateLeave().where(query).sort({ createdAt: -1 }).lean();
    res.json({ leaves });
  } catch (error) { res.status(500).json({ message: 'Failed to load leave applications', error }); }
});
...
router.post('/', async (req: any, res) => {
  try {
    ...
    } else if (req.body.studentId) student = await Student.findOne({ _id: req.body.studentId, institutionId: req.user.institutionId, userId: req.user._id, isActive: { $ne: false } });
    else student = await Student.findOne({ institutionId: req.user.institutionId, userId: req.user._id, isActive: { $ne: false } });
  ...
  } catch (error) { res.status(500).json({ message: 'Failed to submit leave application', error }); }
});
...
router.patch('/:id/review', authorize('admin', 'super_admin', 'head', 'assistant_head', 'class_teacher'), async (req: any, res) => {
  try {
    ...
    if (status === 'approved') {
      for (const date of leaveDates) await Attendance.findOneAndUpdate({ institutionId: req.user.institutionId, studentId: student._id, date }, { institutionId: req.user.institutionId, studentId: student._id, userId: student.userId, userType: 'student', classId: student.classId, sectionId: student.sectionId, date, status: 'leave', notes: `Approved leave: ${leave.reason}`, markedBy: req.user._id, markedAt: new Date() }, { upsert: true, new: true, setDefaultsOnInsert: true });
    } else await Attendance.deleteMany({ institutionId: req.user.institutionId, studentId: student._id, date: { $in: leaveDates }, status: 'leave' });
  ...
  } catch (error) { res.status(500).json({ message: 'Failed to review leave application', error }); }
});
