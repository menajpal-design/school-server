...
const primary = async (fn: () => Promise<any>) => runWithTenantStorage(null, fn);

router.get('/', authenticate, canManageAcademic(), async (req, res) => {
  try {
    const classes = await primary(async () => {
      ...
      return list.map((x: any) => ({ ...x, totalStudents: count.get(String(x._id)) || 0, status: x.isActive ? 'active' : 'inactive' }));
    });
    res.json({ classes });
  } catch (e: any) {
...
router.post('/', authenticate, canManageAcademic(), async (req, res) => {
  try {
    const data = await primary(async () => {
      ...
      return Array.isArray(req.body?.items) || Array.isArray(req.body) ? { classItems: created } : { classItem: created[0] };
    });
