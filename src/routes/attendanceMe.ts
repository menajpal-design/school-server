...
const safeProfile = (req: any) => ({ _id: req.user?._id, userId: { _id: req.user?._id, name: req.user?.name, email: req.user?.email, avatar: req.user?.avatar, role: req.user?.role }, name: req.user?.name, role: req.user?.role, profileMissing: true });
...
router.get('/people', authenticate, async (req: any, res) => {
  try {
    ...
    const people = await Student.find(query).populate('userId', 'name username email phone avatar role').populate('classId', 'name grade').populate('sectionId', 'name').sort({ rollNumber: 1, createdAt: 1 }).lean();
    res.json({ people, lockedClassId, lockedClassIds });
  } catch (error) { res.status(500).json({ message: 'Failed to load attendance people', error }); }
});
...
router.get('/me', authenticate, async (req: any, res) => {
  try {
    ...
    if (role === 'student') {
      ...
      if (!student) return res.json({ attendance: [], profile: safeProfile(req), personType: 'student', month, year, summary: summary([]), message: 'Student profile is not linked yet. Please contact school office.' });
    ...
    if (role === 'parent') {
      ...
      if (!selectedChild || !childIds.includes(selectedChild)) return res.json({ attendance: [], profile: safeProfile(req), children: childIds, personType: 'student', month, year, summary: summary([]), message: 'No child profile is linked with this parent account.' });
      ...
      if (!student) return res.json({ attendance: [], profile: safeProfile(req), children: childIds, personType: 'student', month, year, summary: summary([]), message: 'Child student profile is not found.' });
      const attendance = await Attendance.find({ institutionId, studentId: student._id, userType: 'student', date: dateQuery }).sort({ date: 1 }).lean();
