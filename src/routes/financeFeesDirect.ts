...
const normalizeAmount = (value: any) => Number.isFinite(Number(value)) ? Math.round(Number(value) * 100) / 100 : 0;
...
async function getConnection(req: any) {
  ...
  catch (error: any) { directConnections.delete(uri); const e: any = new Error(`Active Settings MongoDB connection failed for Finance Fee: ${error?.message || 'unknown error'}`); e.statusCode = 503; throw e; }
}
...
async function models(req: any) {
  ...
  return { Fee: model('Fee', Fee), Payment: model('Payment', Payment), Student: model('Student', Student), Class: model('Class', ClassModel), User: model('User', User) };
}
...
router.post('/payments', async (req: any, res) => {
  try {
    ...
    const fee = req.body.feeId ? await M.Fee.findOne({ _id: req.body.feeId, institutionId: req.user.institutionId }) : await M.Fee.findOne({ studentId: req.body.studentId, institutionId: req.user.institutionId, status: { $in: ['pending', 'overdue'] } }).sort({ dueDate: 1 });
    if (!fee) return res.status(404).json({ message: 'No due fee found for payment' });
    const payableAmount = normalizeAmount(fee.amount);
    const paidAmount = normalizeAmount(req.body.amount);
    ...
    const remainingAmount = normalizeAmount(payableAmount - paidAmount);
...
router.get('/', async (req: any, res) => {
  try { const M = await models(req); const fees = await populateFee(M).where({ institutionId: req.user.institutionId }).sort({ createdAt: -1 }).lean(); res.json({ fees, source: 'settings-active-mongodb-direct' }); }
  catch (error: any) { res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to load fees', error }); }
});

router.post('/', async (req: any, res) => {
  try { const M = await models(req); const payload = normalizeFeePayload(req.body); const calculated = calculateFeeAmount(payload); const fee = await M.Fee.create({ ...payload, ...calculated, collectedBy: req.user._id, institutionId: req.user.institutionId }); const created = await populateFee(M).where({ _id: fee._id, institutionId: req.user.institutionId }).findOne().lean(); await writeAuditLog(req, 'create', 'fee', fee._id, created).catch(() => undefined); res.status(201).json({ fee: created, source: 'settings-active-mongodb-direct' }); }
  catch (error: any) { res.status(error?.name === 'ValidationError' || error?.name === 'CastError' ? 400 : error?.statusCode || 500).json({ message: error?.message || 'Failed to create fee', error }); }
});

router.put('/:id', async (req: any, res) => {
  try { const M = await models(req); const payload = normalizeFeePayload(req.body); const calculated = calculateFeeAmount(payload); const fee = await M.Fee.findOneAndUpdate({ _id: req.params.id, institutionId: req.user.institutionId }, { ...payload, ...calculated }, { new: true }); if (!fee) return res.status(404).json({ message: 'Fee not found' }); const updated = await populateFee(M).where({ _id: fee._id, institutionId: req.user.institutionId }).findOne().lean(); await writeAuditLog(req, 'update', 'fee', fee._id, updated).catch(() => undefined); res.json({ fee: updated, source: 'settings-active-mongodb-direct' }); }
  catch (error: any) { res.status(error?.name === 'ValidationError' || error?.name === 'CastError' ? 400 : error?.statusCode || 500).json({ message: error?.message || 'Failed to update fee', error }); }
});

router.delete('/:id', async (req: any, res) => {
  try { const M = await models(req); const fee = await M.Fee.findOne({ _id: req.params.id, institutionId: req.user.institutionId }); if (!fee) return res.status(404).json({ message: 'Fee not found' }); await fee.deleteOne(); await writeAuditLog(req, 'delete', 'fee', fee._id, undefined, fee).catch(() => undefined); res.json({ message: 'Fee deleted' }); }
  catch (error: any) { res.status(error?.statusCode || 500).json({ message: error?.message || 'Failed to delete fee', error }); }
});
