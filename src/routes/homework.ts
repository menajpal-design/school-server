...
const canManageHomeworkClass = async (req: any, classId: any) => { if (roleIn(req.user.role, ['admin', 'super_admin', 'head', 'assistant_head'])) return true; if (!roleIn(req.user.role, teacherRoles)) return false; const assignedClasses = await assignedClassesForTeacher(req); return assignedClasses.length === 0 || assignedClasses.includes(String(classId)); };
...
router.get('/', authenticate, async (req: any, res) => {
  try {
    ...
    if (roleIn(role, managerRoles)) {
      ...
      if (roleIn(role, teacherRoles)) { const assignedClasses = await assignedClassesForTeacher(req); if (assignedClasses.length) query.classId = { $in: assignedClasses }; }
